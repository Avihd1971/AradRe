...
async function db() {
  // In Cloudflare Workers, pull the D1 binding from the request context.
  // In Node.js (local dev / build), getCloudflareContext throws — fall back to SQLite.
  try {
    const { getCloudflareContext } = await import("@opennextjs/cloudflare")
    const { env } = await getCloudflareContext({ async: true })
    if (env.DB) return getDB(env.DB as D1Database)
  } catch {
    // Not in a Cloudflare Workers context — use local SQLite
  }
  return getDB()
}
...
export async function getPropertyById(id: string): Promise<PropertyData | null> {
  const client = await db()
  return client.property.findUnique({ where: { id } })
}
...
export async function toggleFeatured(id: string, value: boolean): Promise<void> {
  const client = await db()
  await client.property.update({ where: { id }, data: { featured: value } })
  revalidatePath("/admin")
  revalidatePath("/")
}
...
export async function toggleStatus(id: string, value: string): Promise<void> {
  const client = await db()
  await client.property.update({ where: { id }, data: { status: value } })
  revalidatePath("/admin")
  revalidatePath("/")
  revalidatePath("/properties")
}
